...
export default function Sidebar() {
  ...
  return (
<div className="hidden xl:flex flex-col  mt-2 mb-3 items-center  fixed right-7  h-full xl:ml-24  rounded-xl ring-2 ring-slate-950 shadow-2xl shadow-slate-950 bg-black   bg-cover bg-center p-5 z-20">  <div className="absolute inset-0 bg-lime-400 translate-y-[56%] ] transition-transform duration-300 rounded-3xl -z-20 " />  
...
{currentUser ? (
    <>
      ...
      <div className=" mt-16 text-gray-700 space-y-7 items-center ">
        <img
          ...
          alt="user-img"
          className="h-150 w-150 rounded-2xl xl:mr-2 ring-2 inset-14 ring-lime-400 p-2 shadow-lg shadow-white mx-12 mt-12 "
        />
        <div className="space-y-7 mt-4">
          <h4 className=" text-4xl text-white font-thin  rounded-md p-2 ">{currentUser?.name}</h4>
    ...
    transition={{duration:3, ease:"anticipate" }} className='text-white   rounded-xl    p-3 flex justify-between '>
        <h1 className=' text-black text-xl font-thin' >Followers</h1><p className=' text-slate-100 text-xl '>10100</p></motion.div>
